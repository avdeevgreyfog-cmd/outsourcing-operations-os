...
export default async function Candidates({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <>
    <PageHeader eyebrow="Подбор" title="Кандидаты" subtitle="База людей и текущая работа по подбору." breadcrumbs={[{label:"Люди"},{label:"Подбор",href:"/recruiting"},{label:"Кандидаты"}]}/>
    <CandidatesWorkspace rows={rows} directory={directory} needs={needs} options={options} initialQueue={initialQueue} demo={actor.demo} canCreate={hasCapability(actor.access,"recruiting.candidate.create")}/>
  </>;
}
