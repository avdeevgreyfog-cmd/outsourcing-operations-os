...
export default async function RequestPage({params}:{params:Promise<{id:string}>}){
  ...
  const acceptedRoles=new Set(calculations.filter(item=>item.status==="accepted").map(item=>item.role));
  const readyForProposal=request.roles.length>0&&request.roles.every(role=>acceptedRoles.has(role.specialty));
  const actions=<>{hasCapability(actor.access,"sales.request.edit")&&<RequestEditButton request={request} options={options} canArchive={hasCapability(actor.access,"sales.request.archive")}/>} {hasCapability(actor.access,"calculation.scenario.create")&&request.status!=="archived"&&<Link href={`/calculations?request=${id}`} className="button primary">Открыть расчёт</Link>}</>;
  return <>
    ...
    <SummaryStrip><span>Этап <Status tone={tone(request.status)}>{request.status}</Status></span><span>Позиции <strong>{request.roles.length}</strong></span><span>Расчёты <strong>{calculations.length}</strong></span><span>Версии КП <strong>{proposals.length}</strong></span></SummaryStrip>
    <div className="workspace-grid"><div>
      ...
      <Section title="Позиции"><div className="grid-scroll"><table className="data-table"><thead><tr><th>Специальность</th><th>Численность</th><th>Текущая согласованная экономика</th></tr></thead><tbody>{request.roles.map(role=><tr key={role.id}><td className="cell-title">{role.specialty}</td><td className="num">{role.count}</td><td><Status tone={acceptedRoles.has(role.specialty)?"good":"warn"}>{acceptedRoles.has(role.specialty)?"accepted":"calculation"}</Status></td></tr>)}</tbody></table></div></Section>
    </div><div>
      <Section title="Расчёты" note="Согласованные сценарии не перезаписываются; при изменении условий создаётся новый сценарий."><div className="stack-list">{calculations.length?calculations.map(x=><div className="stack-item" key={x.id}><div><strong>{x.name}</strong><small>{x.role} · {x.model}</small></div><Status tone={tone(x.status)}>{x.status}</Status></div>):<div className="empty-inline">Расчётов пока нет</div>}</div>{hasCapability(actor.access,"calculation.scenario.create")&&<div style={{padding:12}}><Link href={`/calculations?request=${id}`} className="button">Создать / пересчитать</Link></div>}</Section>
