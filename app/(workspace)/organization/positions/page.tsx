...
import { listOrganizationUnits, listPositionAssignments, listPositions, listProcessRoles, listResponsibilityRules, listStaffPositions } from "@/lib/organization/service";
...
export default async function PositionsPage(){
  const actor=await requireActor();const [profiles,staffPositions,assignments,roles,responsibilities,units]=await Promise.all([listPositions(actor),listStaffPositions(actor),listPositionAssignments(actor),listProcessRoles(actor),listResponsibilityRules(actor),listOrganizationUnits(actor)]);const canManage=hasCapability(actor.access,"organization.position.manage");
