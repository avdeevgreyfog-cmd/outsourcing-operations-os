...
export default async function OrganizationStructurePage(){
  ...
  return <><PageHeader eyebrow="Организация" title="Оргструктура" subtitle="Текущая структура подразделений, штатных позиций и назначений сотрудников." breadcrumbs={[{label:"Организация"},{label:"Структура"},{label:"Оргструктура"}]} actions={<OrganizationCreatePanel kind="unit" canManage={canManage} demo={actor.demo} units={units} employees={employees}/>}/><OrganizationTabs active="/organization/structure"/><SummaryStrip><span>Компания <strong>{company.name}</strong></span><span>Подразделения <strong>{units.length}</strong></span><span>Сотрудники <strong>{employees.length}</strong></span><span>Штатные позиции <strong>{staffPositions.reduce((sum,item)=>sum+item.capacity,0)}</strong></span><span>Вакансии <strong>{staffPositions.reduce((sum,item)=>sum+item.open,0)}</strong></span></SummaryStrip><OrganizationChart units={units} employees={employees} staffPositions={staffPositions} assignments={assignments}/></>;
}
