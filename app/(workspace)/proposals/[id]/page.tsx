...
export default async function ProposalPage({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    </div><aside><Section title="Workflow" note="Внутреннее согласование отделено от решения клиента."><div style={{padding:12}}><ProposalWorkflowActions proposalId={proposal.id} status={proposal.status} objectId={proposal.sourceObjectId}/></div></Section><Section title="Историчность"><div style={{padding:"6px 15px 14px"}}><KeyValue label="Согласовано внутри" value={proposal.approvedAt??"—"}/><KeyValue label="Отправлено клиенту" value={proposal.sentAt??"—"}/><KeyValue label="Принято клиентом" value={proposal.acceptedAt??"—"}/><KeyValue label="Передано в запуск" value={proposal.launchedAt??"—"}/></div></Section></aside></div>
  </>;
}
