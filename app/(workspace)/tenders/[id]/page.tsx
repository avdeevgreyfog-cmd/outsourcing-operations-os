...
import {TenderApprovalActions,TenderComments,TenderCoreEditor,TenderDocumentsPanel,TenderSubmissionEditor,TenderTeamEditor} from "@/components/TenderEntityPanels";
...
export default async function TenderPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{tab?:string}>}){
  ...
  return <>
    ...
    {active==="overview"&&<>
      ...
      </div><aside className="request-entity-side"><Section title="Рабочий контур"><div className="request-entity-side-body"><Status tone={tone(tender.stage)}>{tenderStageLabel(tender.stage)}</Status><KeyValue label="Ответственный" value={tender.owner??"Не назначен"}/><KeyValue label="Решение" value={tenderDecisionLabels[tender.decision]??tender.decision}/><KeyValue label="Потенциал" value={tender.potential==="high"?"Высокий":tender.potential==="low"?"Низкий":"Средний"}/><KeyValue label="Приоритет" value={tender.priority==="high"?"Высокий":tender.priority==="low"?"Низкий":"Обычный"}/><KeyValue label="Блокеры" value={String(tender.blockerCount)}/><KeyValue label="Расчёты" value={String(tender.calculationCount)}/>{tender.result&&<KeyValue label="Результат" value={tenderResultLabels[tender.result]??tender.result}/>}</div></Section><TenderCoreEditor tender={tender} options={options} canEdit={canEdit} canResult={canResult}/><TenderTeamEditor tender={tender} options={options} canEdit={canEdit}/></aside></div>
    </>}
    {active==="analysis"&&<TenderAnalysisWorkspace tender={tender} options={options} canEdit={canEdit}/>} 
    {active==="documents"&&<TenderDocumentsPanel tender={tender} options={options} canEdit={canEdit}/>} 
    {active==="calculations"&&<Section title="Экономика тендера" actions={hasCapability(actor.access,"calculation.scenario.create")&&tender.roles.length?<Link className="button primary" href={`/calculations?tender=${id}`}>Новый расчёт</Link>:undefined}>{tender.calculations.length?<div className="grid-scroll"><table className="data-table"><thead><tr><th>Позиция</th><th>Сценарий</th><th>Модель</th><th>Ставка без НДС</th><th>С НДС</th><th>Маржа</th><th>Статус</th></tr></thead><tbody>{tender.calculations.map(item=><tr key={item.scenarioId}><td>{item.role}</td><td><Link href={`/calculations?tender=${id}#scenario-${item.scenarioId}`}>{item.name}</Link></td><td>{item.model}</td><td>{money(item.clientRate)}</td><td>{item.clientRateGross==null?"—":money(item.clientRateGross)}</td><td>{Number(item.marginPct).toFixed(1)}%</td><td>{item.status}</td></tr>)}</tbody></table></div>:<p className="muted">Расчётов пока нет. Добавьте позиции во вкладке «Анализ» и откройте общий калькулятор OPERIS.</p>}</Section>}
    {active==="approvals"&&<div className="tender-tab-stack"><TenderApprovalActions tender={tender} canEdit={canEdit}/><Section title="История согласований">{tender.approvals.length?<div className="grid-scroll"><table className="data-table"><thead><tr><th>Процесс</th><th>Инициатор</th><th>Согласующий</th><th>Отправлено</th><th>Статус</th><th>Комментарий</th></tr></thead><tbody>{tender.approvals.map(item=><tr key={item.id}><td>{item.processCode==="tender_participation"?"Участие":item.processCode==="tender_bid"?"Цена":"Подача"}</td><td>{item.requestedBy}</td><td>{item.approver??"—"}</td><td>{item.requestedAt}</td><td>{item.status}</td><td>{item.decisionComment??"—"}</td></tr>)}</tbody></table></div>:<p className="muted">Согласований пока нет.</p>}</Section></div>}
