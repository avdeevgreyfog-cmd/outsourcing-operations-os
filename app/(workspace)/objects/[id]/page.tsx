  ...
  return <>
    ...
    {tab==="workforce"&&<>
      ...
      <Section title="Сотрудники">
        <div className="request-table-wrap"><table className="data-table"><thead><tr><th>Сотрудник</th><th>Специальность</th><th>Начало работы</th><th>Оформление</th><th>Источник</th><th>Статус</th></tr></thead><tbody>{objectWorkers.map(row=><tr key={row.id}><td><Link className="cell-title" href={"/workers/"+row.id}>{row.fullName}</Link></td><td>{row.specialty??"—"}</td><td>{row.startDate?new Intl.DateTimeFormat("ru-RU").format(new Date(row.startDate+"T00:00:00")):"—"}</td><td>{row.employment??"—"}</td><td>{row.origin??row.source??"—"}</td><td><Status tone={row.status==="active"?"good":"neutral"}>{row.status==="active"?"Работает":row.status==="dismissed"?"Работа завершена":row.status}</Status></td></tr>)}</tbody></table></div>
