...
const schema=z.object({subjectType:z.enum(["calculation_scenario","proposal"]),subjectId:z.string().uuid()});

type SubjectContext={requestId:string;regionId:string|null;organizationId:string;ownerUserId:string|null;createdByUserId:string;teamId:string|null;clientId:string|null;status:string};
...
export async function POST(request:Request){
  try{
    ...
    const result=await withTenant(actor.organizationId,actor.userId,async sql=>sql.begin(async tx=>{
      ...
      let processCode="";let stepCode="";let capability="";
      if(body.subjectType==="calculation_scenario"){
        capability="calculation.scenario.edit";processCode="commercial_calculation";stepCode="calculation_approval";
        requireCapability(actor,capability);
        const [row]=await tx<Array<SubjectContext>>`
          SELECT r.id "requestId",r.region_id "regionId",r.organization_id "organizationId",r.owner_user_id "ownerUserId",
            r.created_by_user_id "createdByUserId",r.assigned_team_id "teamId",r.client_company_id "clientId",cs.status
          FROM calculation_scenarios cs JOIN calculations c ON c.id=cs.calculation_id JOIN requests r ON r.id=c.request_id
          WHERE cs.id=${body.subjectId}::uuid
        `;context=row;
        ...
        capability="sales.proposal.submit";processCode="commercial_proposal";stepCode="proposal_approval";
        requireCapability(actor,capability);
        const [row]=await tx<Array<SubjectContext>>`
          SELECT r.id "requestId",r.region_id "regionId",r.organization_id "organizationId",r.owner_user_id "ownerUserId",
            r.created_by_user_id "createdByUserId",r.assigned_team_id "teamId",r.client_company_id "clientId",p.status
      ...
      if(pending)throw new Error("По этой версии уже есть активное согласование");

      const [approver]=await tx<Array<{membershipId:string;userId:string}>>`
        WITH direct_rule AS (
          SELECT membership_id "membershipId",user_id "userId",1 priority
      ...
      const [approval]=await tx<Array<{id:string}>>`
        ...
        VALUES(${actor.organizationId}::uuid,${body.subjectType},${body.subjectId}::uuid,${processCode},'pending',${actor.userId}::uuid,${sql.json({requestId:context.requestId,regionId:context.regionId})}) RETURNING id
      `;
      await tx`
        INSERT INTO approval_steps(organization_id,approval_id,step_order,step_code,status,approver_membership_id,approver_user_id)
        VALUES(${actor.organizationId}::uuid,${approval.id}::uuid,1,${stepCode},'pending',${approver.membershipId}::uuid,${approver.userId}::uuid)
      `;
      if(body.subjectType==="calculation_scenario"){
        ...
        await tx`UPDATE requests SET status='calculation_review',updated_at=now() WHERE id=${context.requestId}::uuid`;
      }else{
        await tx`UPDATE proposals SET status='internal_review',submitted_at=now() WHERE id=${body.subjectId}::uuid`;
        await tx`UPDATE requests SET status='proposal_review',updated_at=now() WHERE id=${context.requestId}::uuid`;
      }
