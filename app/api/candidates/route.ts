  ...
  try {
    ...
    const result = await withTenant(actor.organizationId, actor.userId, async sql => sql.begin(async tx => {
      ...
      const [application] = await tx<Array<{id:string}>>`
        ...
        VALUES(${actor.organizationId}::uuid,${candidateId}::uuid,${body.needId}::uuid,${need.objectId}::uuid,'new',${body.nextActionAt??null}::timestamptz,${need.ownerUserId??actor.userId}::uuid,${need.managerUserId}::uuid,${sql.json(need.conditions??{})},${actor.userId}::uuid)
        RETURNING id
      `;
