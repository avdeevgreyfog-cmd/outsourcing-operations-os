...
const schema=z.object({specialtyId:z.string().uuid()});
  ...
  try{
    ...
    await withTenant(actor.organizationId,actor.userId,async sql=>sql.begin(async tx=>{
      ...
      const rows=await tx<Array<{id:string}>>`
        UPDATE object_ppe_templates
        SET active=false,updated_by_user_id=${actor.userId}::uuid,updated_at=now()
        WHERE object_id=${id}::uuid AND specialty_id=${body.specialtyId}::uuid AND active
        RETURNING id
      `;
      await tx`
        INSERT INTO activity_events(organization_id,actor_user_id,entity_type,entity_id,verb,summary,metadata)
        VALUES(${actor.organizationId}::uuid,${actor.userId}::uuid,'object',${id}::uuid,'ppe_template_reset','Норма обеспечения объекта сброшена к базовой',${tx.json({specialtyId:body.specialtyId,overrideIds:rows.map(row=>row.id)})})
      `;
    }));
    return NextResponse.json({ok:true});
  }catch(error){
    if(error instanceof z.ZodError)return NextResponse.json({error:"Проверьте специальность",issues:error.issues},{status:400});
    if(error instanceof AccessDeniedError)return NextResponse.json({error:"Недостаточно прав"},{status:403});
    console.error(error);return NextResponse.json({error:error instanceof Error?error.message:"Не удалось сбросить норму"},{status:500});
  }
}
