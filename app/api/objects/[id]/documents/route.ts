...
const schema=z.object({name:z.string().trim().min(2).max(300),category:z.enum(["client_instruction","access","ppe","safety","act","template","client","other"]).default("other"),documentNumber:z.string().trim().max(160).nullable().optional(),documentDate:z.string().date().nullable().optional(),versionLabel:z.string().trim().min(1).max(80).nullable().optional(),sourceUrl:z.string().url().max(2000).nullable().optional(),status:z.enum(["active","needs_update","archived"]).default("active"),validFrom:z.string().date().nullable().optional(),expiresAt:z.string().date().nullable().optional(),notes:z.string().trim().max(3000).nullable().optional()});
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const created=await withTenant(actor.organizationId,actor.userId,async sql=>sql.begin(async tx=>{
      ...
      const [row]=await tx<Array<{id:string}>>`INSERT INTO object_documents(organization_id,object_id,name,category,document_number,document_date,version_label,source_url,status,valid_from,expires_at,notes,created_by_user_id,updated_by_user_id) VALUES(${actor.organizationId}::uuid,${id}::uuid,${body.name},${body.category},${body.documentNumber??null},${body.documentDate??null}::date,${body.versionLabel??null},${body.sourceUrl??null},${body.status},${body.validFrom??null}::date,${body.expiresAt??null}::date,${body.notes??null},${actor.userId}::uuid,${actor.userId}::uuid) RETURNING id`;
