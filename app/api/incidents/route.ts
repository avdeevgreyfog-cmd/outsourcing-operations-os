...
const schema=z.object({
  ...
  occurredAt:z.string().datetime(),financialEffectAmount:z.number().min(0).max(10000000).nullable().optional(),financialEffectBasis:z.string().trim().max(1000).nullable().optional(),
});
  ...
  try{
    ...
    const row=await withTenant(actor.organizationId,actor.userId,async sql=>sql.begin(async tx=>{
      ...
      const [created]=await tx<Array<{id:string}>>`
        INSERT INTO incidents(organization_id,object_id,worker_id,incident_type,severity,status,title,description,occurred_at,responsible_user_id,financial_effect_amount,financial_effect_status,financial_effect_basis,created_by_user_id)
        VALUES(${actor.organizationId}::uuid,${body.objectId}::uuid,${body.workerId??null}::uuid,${body.incidentType},${body.severity},'open',${body.title},${body.description},${body.occurredAt}::timestamptz,${actor.userId}::uuid,${body.financialEffectAmount??null},${financialStatus},${body.financialEffectBasis??null},${actor.userId}::uuid)
        RETURNING id
      `;
