...
const schema=z.object({
  ...
  roles:z.array(role).max(40).default([]),
});
    ...
    const result=await withTenant(actor.organizationId,actor.userId,async sql=>sql.begin(async tx=>{
      ...
      const [r]=await tx<Array<{id:string;title:string;status:string}>>`
        INSERT INTO requests (
          organization_id,client_company_id,title,status,source,location_text,region_id,expected_start_date,duration_text,schedule_json,intake_json,lunch_paid,
