...
const createSchema=z.object({name:z.string().trim().min(2).max(160),email:z.string().email().max(320),phone:z.string().trim().max(40).optional(),positionId:z.string().uuid().nullable().optional(),orgUnitId:z.string().uuid().nullable().optional(),managerMembershipId:z.string().uuid().nullable().optional(),processRoleIds:z.array(z.string().uuid()).max(30).optional()});
const updateSchema=createSchema.partial().extend({id:z.string().uuid(),status:z.enum(["active","invited","suspended"]).optional()});
...
export async function POST(request:Request){
  try{
    ...
    const membershipId=await withTenant(actor.organizationId,actor.userId,async sql=>{
      if(body.positionId&&!((await sql.unsafe<Array<{id:string}>>("SELECT id FROM positions WHERE id=$1::uuid",[body.positionId]))[0]))throw new Error("INVALID_ASSIGNMENT");
      if(body.orgUnitId&&!((await sql.unsafe<Array<{id:string}>>("SELECT id FROM organization_units WHERE id=$1::uuid",[body.orgUnitId]))[0]))throw new Error("INVALID_ASSIGNMENT");
      ...
      const [membership]=await sql.unsafe<Array<{id:string}>>(
        ...
        [actor.organizationId,userId,roleTemplate.id,body.positionId??null,body.orgUnitId??null,body.managerMembershipId??null,body.phone??null]
      );
...
export async function PATCH(request:Request){
  try{
    ...
    await withTenant(actor.organizationId,actor.userId,async sql=>{
      ...
      await sql.unsafe(
        "UPDATE organization_memberships SET position_id=CASE WHEN $2::boolean THEN position_id ELSE $3::uuid END,primary_org_unit_id=CASE WHEN $4::boolean THEN primary_org_unit_id ELSE $5::uuid END,manager_membership_id=CASE WHEN $6::boolean THEN manager_membership_id ELSE $7::uuid END,phone=COALESCE($8,phone),status=COALESCE($9,status) WHERE id=$1::uuid",
        [body.id,body.positionId===undefined,body.positionId??null,body.orgUnitId===undefined,body.orgUnitId??null,body.managerMembershipId===undefined,body.managerMembershipId??null,body.phone??null,body.status??null]
      );
