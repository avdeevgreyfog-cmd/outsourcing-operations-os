...
export function ProposalWorkflowActions({proposalId,status,objectId,canSubmit,canClientDecision,canLaunch,legalEntities=[]}:{proposalId:string;status:string;objectId?:string|null;canSubmit:boolean;canClientDecision:boolean;canLaunch:boolean;legalEntities?:Array<{id:string;name:string;shortName:string|null;primary:boolean}>}){
  const [busy,setBusy]=useState(false);const [error,setError]=useState("");const [note,setNote]=useState("");const [launchOpen,setLaunchOpen]=useState(false);const [name,setName]=useState("");const [code,setCode]=useState("");const [legalEntityId,setLegalEntityId]=useState(()=>legalEntities.find(item=>item.primary)?.id??legalEntities[0]?.id??"");const router=useRouter();
  ...
  async function launch(){try{setBusy(true);setError("");if(!legalEntityId)throw new Error("Выберите юридическое лицо");const result=await api(`/api/proposals/${proposalId}/launch`,"POST",{name:name||undefined,code:code||undefined,legalEntityId});setLaunchOpen(false);router.push(`/objects/${result.id}`);router.refresh()}catch(e){setError(e instanceof Error?e.message:"Ошибка подготовки")}finally{setBusy(false)}}
  ...
  return <div className="proposal-actions">
    ...
    {launchOpen&&<><div className="drawer-backdrop" onClick={()=>setLaunchOpen(false)}/><aside className="drawer"><button className="icon-button drawer-close" onClick={()=>setLaunchOpen(false)}>×</button><span className="eyebrow">Переход в подготовку</span><h2>Начать подготовку объекта</h2><div className="login-form" style={{marginTop:18}}><label>Название объекта<input value={name} onChange={e=>setName(e.target.value)} placeholder="По умолчанию — название заявки"/></label><label>Код объекта<input value={code} onChange={e=>setCode(e.target.value)} placeholder="Автоматически, если пусто"/></label><label>Наше юрлицо<select value={legalEntityId} onChange={e=>setLegalEntityId(e.target.value)}><option value="">Выберите юрлицо</option>{legalEntities.map(item=><option key={item.id} value={item.id}>{item.shortName??item.name}</option>)}</select></label><p className="form-hint">Одним действием будут созданы объект в статусе подготовки, потребности по всем позициям заявки, план запуска с базовыми задачами, клиентские ставки из принятой экономики и черновик договора. Подбор и подготовка запуска начнутся параллельно; фактический запуск останется заблокирован до подписания договора или отдельного разрешения.</p><button className="button primary" disabled={busy} onClick={launch}>{busy?"Создание…":"Начать подготовку"}</button>{error&&<div className="form-error">{error}</div>}</div></aside></>}
  </div>;
}
