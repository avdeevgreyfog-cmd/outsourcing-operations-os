  ...
  return <div>
    ...
    <section className="section section-flush"><div className="request-table-wrap"><table className="data-table">
      ...
      <tbody>{grouped.map(row=>{const key=`${row.locationId}:${row.itemId}:${row.variant}`;const lowStock=row.minQuantity>0&&row.quantity<=row.minQuantity;return <tr key={key}>
        ...
        <td>{canManage?<div className="inventory-limit-control"><input type="number" min="0" value={limitDrafts[key]??String(row.minQuantity)} onChange={e=>setLimitDrafts(current=>({...current,[key]:e.target.value}))}/><button className="button" disabled={busy} onClick={()=>void saveLimit(row)}>Сохранить</button></div>:row.minQuantity}</td>
    ...
    {showMovement&&<Portal><div className="recruiting-modal" onMouseDown={e=>{if(e.currentTarget===e.target)setShowMovement(false)}}><div className="recruiting-modal-card">
      ...
      {type==="return"&&<label className="operations-check"><input type="checkbox" checked={writeoffAfterReturn} onChange={e=>setWriteoffAfterReturn(e.target.checked)}/> Списать сразу после возврата</label>}
