...
export function RequestEditButton({request,options,canArchive}:{request:CommercialRequestDetail;options:CommercialOptions;canArchive:boolean}){
  ...
  return <><button className="button" onClick={()=>setOpen(true)}>Редактировать</button>{open&&<><div className="drawer-backdrop" onClick={()=>setOpen(false)}/><aside className="drawer commercial-drawer"><button className="icon-button drawer-close" onClick={()=>setOpen(false)}>×</button><span className="eyebrow">Заявка</span><h2>{request.title}</h2><RequestForm options={options} request={request} roles={roles} setRoles={setRoles} onSubmit={submit} error={error}/>{canArchive&&<div className="drawer-danger-zone"><button type="button" className="button" onClick={()=>archive(request.status==="archived"?"restore":"archive")}>{request.status==="archived"?"Восстановить заявку":"Архивировать заявку"}</button></div>}</aside></>}</>;
}
