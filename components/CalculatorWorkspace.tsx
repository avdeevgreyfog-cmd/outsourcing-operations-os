...
const billingLabels: Record<BillingUnit, string> = {
  hour: "₽ / час", shift: "₽ / смена", unit: "₽ / единица", worker_month: "₽ / сотрудник / месяц",
  project_month: "₽ / проект / месяц", project_fixed: "Фикс за проект", mixed: "Фикс + переменная часть",
};
const simpleBillingLabels: Record<Exclude<BillingUnit, "mixed" | "project_fixed">, string> = {
  hour: "час", shift: "смена", unit: "единица", worker_month: "сотрудник / месяц", project_month: "проект / месяц",
};
const warningLabels: Record<string, string> = {
  below_minimum_margin: "Маржа ниже минимального норматива компании", above_client_limit: "Расчётная ставка выше указанного лимита клиента",
  rules_unverified: "Активная версия правил помечена как непроверенная", rule_version_missing: "Для модели не найдена действующая версия правил",
};
...
export function CalculatorWorkspace({ context }: { context?: Context }) {
  ...
  const result = useMemo(() => calculateCommercialScenario({
    workers, hoursPerWorker: hours, hoursPerShift: shiftHours, shiftsPerWorker: shifts, projectMonths, workerPayAmount, workerPayUnit,
    pricingMode, targetMarginPct: margin, clientLimit: clientLimit || null, clientLimitVatMode, billingUnit, variableBillingUnit, unitsPerWorkerShift,
    fixedMonthlyNet, minimumMonthlyNet, vatMode, vatPct, roundingStep: model?.rules.roundingStep, rules: model?.rules ?? {},
    ruleVersionId: model?.ruleVersionId ?? null, costs,
  ...
  function selectRole(nextId: string) { setSelectedRoleId(nextId); const role=context?.roles.find((item)=>item.id===nextId); if(!role)return; setWorkers(role.count); const paidHours=num(role.schedule?.paidHours??context?.schedule?.paidHours,shiftHours); setShiftHours(paidHours);setHours(paidHours*shifts);setClientLimit(Number(role.targetClientRate??0)); }
  function exportSummary() {
    const lines: Array<Array<string | number>> = [["Показатель","Значение"],["Модель",model?.name??"—"],["Версия правил",model?.ruleVersion??"—"],["Способ цены",pricingMode==="target_margin"?"От целевой маржи":"От лимита клиента"],["Единица тарификации",billingLabels[billingUnit]],["Себестоимость / час",result.totalCostHourly],["Ставка клиенту без НДС",result.clientRateNet],["Ставка клиенту с НДС",result.clientRateGross],["Маржа, %",result.marginPct],["Выручка без НДС / месяц",result.monthlyRevenueNet],["Прибыль / месяц",result.monthlyContribution]];
    const csv=lines.map((row)=>row.map((value)=>`"${String(value).replaceAll('"','""')}"`).join(";")).join("\n");const link=document.createElement("a");link.href=URL.createObjectURL(new Blob(["\ufeff"+csv],{type:"text/csv;charset=utf-8"}));link.download="расчёт-сценария.csv";link.click();URL.revokeObjectURL(link.href);
  }
  async function saveScenario() {
    if(!context||!sourceId||!selectedRoleId||!model)return;setSaveState({busy:true,message:"",error:false});
    const inputs={workerPayAmount,workerPayUnit,workers,hoursPerWorker:hours,shiftHours,shiftsPerWorker:shifts,projectMonths,pricingMode,targetMarginPct:margin,clientLimit:clientLimit||null,clientLimitVatMode,billingUnit,variableBillingUnit,unitsPerWorkerShift,fixedMonthlyNet,minimumMonthlyNet,vatMode,vatPct,model:model.code,ruleVersionId:model.ruleVersionId};
    const response=await fetch("/api/calculations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sourceType,sourceId,sourceRoleId:selectedRoleId,modelId:model.id,ruleVersionId:model.ruleVersionId??undefined,name:scenarioName,inputs,costs,result})});
    const json=await response.json().catch(()=>({}));if(!response.ok){setSaveState({busy:false,message:json.error??"Не удалось сохранить сценарий",error:true});return;}setSaveState({busy:false,message:"Сценарий сохранён. Его можно отправить на согласование.",error:false});router.refresh();
  }
  ...
  return <div className="calc-layout"><section className="section">
    ...
    <details className="calc-group" open><summary><span>Тарификация клиента</span><span>{billingLabels[billingUnit]}</span></summary>
      ...
      <div className="calc-row"><span>✓</span><span>Режим НДС</span><select value={vatMode} onChange={(event)=>setVatMode(event.target.value)}><option value="with_vat">С НДС</option><option value="without_vat">Без НДС</option><option value="not_applicable">Не применяется</option></select><span></span></div>{vatMode==="with_vat"&&<CalcInput label="Ставка НДС из правил" value={vatPct} onChange={setVatPct} unit="%"/>}
    </details>

    {groups.map((group)=>{const items=costs.filter((cost)=>cost.group===group);return <details className="calc-group" open key={group}><summary><span>{group}</span><span>{items.filter((item)=>item.enabled).length} стат.</span></summary>{items.map((cost)=><div className="calc-row calc-row-full" key={cost.id}><input type="checkbox" checked={cost.enabled} onChange={(event)=>update(cost.id,{enabled:event.target.checked})} aria-label={`Включить ${cost.label}`}/><input className="expense-name" value={cost.label} onChange={(event)=>update(cost.id,{label:event.target.value})}/><input type="number" value={cost.amount} onChange={(event)=>update(cost.id,{amount:Number(event.target.value)})}/><select value={cost.base} onChange={(event)=>update(cost.id,{base:event.target.value as Base})}><option value="per_hour">₽/ч/сотрудник</option><option value="per_shift">₽/смену/сотрудник</option><option value="per_worker_month">₽/чел./мес</option><option value="project_month">₽/проект/мес</option><option value="project_fixed">₽/проект разово</option><option value="per_unit">₽/единицу</option></select><button type="button" onClick={()=>duplicate(cost)} aria-label="Дублировать"><Copy size={14}/></button><button type="button" onClick={()=>setCosts((value)=>value.filter((item)=>item.id!==cost.id))} aria-label="Удалить"><Trash2 size={14}/></button></div>)}<button type="button" className="add-expense" onClick={()=>add(group)}><Plus size={14}/> Добавить статью</button></details>;})}
  </section><aside className="section calc-summary"><div className="calc-result">
    <div className="eyebrow">Сценарий · {model?.name??"Модель"}</div><div className="big">{rub(result.clientRateNet)}<small> {rateSuffix} без НДС</small></div>
    {vatMode==="with_vat"&&<div className="calc-kv"><span>Ставка с НДС</span><strong>{rub(result.clientRateGross)} {rateSuffix}</strong></div>}
    <div className="calc-kv"><span>Сотруднику на руки / месяц</span><strong>{rub(result.workerPayMonthly)}</strong></div><div className="calc-kv"><span>Обязательные начисления / месяц</span><strong>{rub(result.mandatoryChargesMonthly)}</strong></div><div className="calc-kv"><span>Дополнительные расходы / месяц</span><strong>{rub(result.additionalCostsMonthly)}</strong></div><div className="calc-kv"><span>Себестоимость / час</span><strong>{rub(result.totalCostHourly)}</strong></div><div className="calc-kv"><span>Точка безубыточности</span><strong>{rub(result.breakEvenRateNet)}</strong></div><div className="calc-kv"><span>Фактическая маржа</span><strong>{pct(result.marginPct)}</strong></div><div className="calc-kv"><span>Выручка без НДС / месяц</span><strong>{rub(result.monthlyRevenueNet)}</strong></div><div className="calc-kv"><span>Прибыль / месяц</span><strong>{rub(result.monthlyContribution)}</strong></div><div className="calc-kv"><span>Прибыль за расчётный срок</span><strong>{rub(result.projectContribution)}</strong></div>
    ...
    <button className="button" style={{width:"100%",marginTop:8}} type="button" onClick={exportSummary}><Download size={14}/> Экспортировать сводку</button>{saveState.message&&<p className={saveState.error?"form-error":"form-success"}>{saveState.message}</p>}<p className="calc-disclaimer">Согласованный сценарий остаётся неизменяемой исторической версией. Новый пересчёт всегда сохраняется отдельным сценарием.</p>
  </div></aside></div>;
}
