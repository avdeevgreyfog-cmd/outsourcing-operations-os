...
export function ObjectShiftsWorkspace({objectId,rows,workers,today}:{objectId:string;rows:ShiftRow[];workers:WorkerRow[];today:string}){
  ...
  return <>
    ...
    {selected.length?<div className="object-shift-list">{selected.map(row=><ShiftRowView key={row.id} row={row} workers={workers} today={today} selectedDate={date}/>)}</div>:<div className="object-shift-empty"><strong>На эту дату смены не сформированы</strong><span>Создайте план смен или выберите другую дату.</span></div>}
    <div className="section-actions"><Link className="button" href={`/timesheets?object=${objectId}`}>Открыть табель</Link><Link className="button primary" href={`/shifts?object=${objectId}`}>Открыть графики и смены</Link></div>
  </>;
}
...
function ShiftRowView({row,workers,today,selectedDate}:{row:ShiftRow;workers:WorkerRow[];today:string;selectedDate:string}){
  ...
  return <details className="object-shift-card"><summary><div><strong>{row.specialty}</strong><span>{shiftKind(row.kind)} · {row.time}</span></div><div className="object-shift-numbers"><span>План <b>{row.demand}</b></span><span>Назначено <b>{row.assigned}</b></span><span>Подтверждено <b>{row.confirmed??0}</b></span>{selectedDate===today&&<span>Вышли <b>{onShift}</b></span>}<span>Резерв <b>{row.reserve}</b></span><Status tone={row.deficit?"warn":"good"}>{row.deficit?`−${row.deficit}`:"План закрыт"}</Status></div></summary><div className="object-shift-people">{assigned.map(worker=>{const day=selectedDate===today?workerTodayStatus(worker,today):null;return <div key={worker.id}><Link href={`/workers/${worker.id}`}>{worker.fullName}</Link><span>{worker.phone??"Телефон не указан"}</span>{day?<Status tone={day.tone}>{day.label}</Status>:<Status tone="info">Назначен</Status>}</div>})}{reserve.map(worker=><div key={`reserve-${worker.id}`}><Link href={`/workers/${worker.id}`}>{worker.fullName}</Link><span>{worker.phone??"Телефон не указан"}</span><Status tone="info">Резерв</Status></div>)}{!assigned.length&&!reserve.length&&<div className="empty-inline">Сотрудники на смену ещё не назначены</div>}{selectedDate===today&&Boolean(noShows)&&<small className="priority-critical">Невыходы: {noShows}</small>}</div></details>;
}
...
function shiftKind(value:string){return ({day:"День",night:"Ночь",mixed:"Смешанная",День:"День",Ночь:"Ночь"} as Record<string,string>)[value]??value}
