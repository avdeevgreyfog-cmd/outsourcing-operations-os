...
export function RecruitingNeedsAnalytics({data,options,needs,metricPreferences,canConfigure,demo}:Props){
  ...
  return <div className="needs-analytics-screen">
    ...
    {showMetrics&&<div className="recruiting-modal" onMouseDown={event=>{if(event.currentTarget===event.target)setShowMetrics(false)}}><div className="recruiting-modal-card needs-metric-settings"><div className="recruiting-modal-head"><div><h2>Настроить показатели</h2><p>Формулы остаются системными. Можно менять состав, порядок, название и цель показателя.</p></div><button type="button" className="icon-button" onClick={()=>setShowMetrics(false)}><X size={17}/></button></div><div className="needs-metric-settings-body">{metricError&&<div className="recruiting-error">{metricError}</div>}<div className="needs-metric-settings-list">{draftPreferences.map((item,index)=>{const definition=recruitingMetricDefinition(item.key);return <div className="needs-metric-setting-row" key={item.key}><label className="needs-metric-visible"><input type="checkbox" checked={item.visible} onChange={event=>setDraftPreferences(current=>current.map(row=>row.key===item.key?{...row,visible:event.target.checked}:row))}/><span>{definition.label}</span></label><input className="needs-metric-label-input" value={item.label} onChange={event=>setDraftPreferences(current=>current.map(row=>row.key===item.key?{...row,label:event.target.value}:row))} aria-label={`Название: ${definition.label}`}/><input className="needs-metric-target-input" type="number" step="0.1" value={item.targetValue??""} onChange={event=>setDraftPreferences(current=>current.map(row=>row.key===item.key?{...row,targetValue:event.target.value===""?null:Number(event.target.value)}:row))} placeholder="Цель" aria-label={`Цель: ${definition.label}`}/><span className="needs-metric-description">{definition.description}</span><div className="needs-metric-order"><button type="button" className="icon-button" disabled={index===0} onClick={()=>moveMetric(index,-1)}><ChevronUp size={14}/></button><button type="button" className="icon-button" disabled={index===draftPreferences.length-1} onClick={()=>moveMetric(index,1)}><ChevronDown size={14}/></button></div></div>})}</div></div><div className="recruiting-form-actions"><button type="button" className="button" onClick={()=>{const reset=recruitingMetricCatalog.map(def=>({key:def.key,label:def.label,visible:def.defaultVisible,position:def.defaultPosition,targetValue:null}));setDraftPreferences(reset)}}>Сбросить к стандарту</button><button type="button" className="button" onClick={()=>setShowMetrics(false)}>Отмена</button><button type="button" className="button primary" disabled={savingMetrics} onClick={()=>void saveMetricSettings()}>{savingMetrics?"Сохраняю…":"Сохранить"}</button></div></div></div>}
  </div>;
}
