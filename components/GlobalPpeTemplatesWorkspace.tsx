...
export function GlobalPpeTemplatesWorkspace({templates,inventoryItems,specialties,canManage,demo}:{templates:ObjectPpeTemplateRow[];inventoryItems:InventoryItemRow[];specialties:SpecialtyOption[];canManage:boolean;demo:boolean}){
  ...
  async function save(){
    ...
    try{
      const items=draft.map(row=>({itemId:row.itemId,quantity:Math.max(.001,Number(row.quantity)||1),sizeSource:row.sizeSource,variant:"",replacementCycleDays:row.replacementCycleDays?Math.max(1,Number(row.replacementCycleDays)):null}));
      if(!demo){
        const response=await fetch("/api/assets/ppe-template",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify({specialtyId,items})});
        const json=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(json.error??"Не удалось сохранить базовую норму");
        window.location.reload();
      }else setMessage("Демо: базовая норма сохранена локально");
    }catch(error){setMessage(error instanceof Error?error.message:"Не удалось сохранить базовую норму")}
    finally{setBusy(false)}
  }

  if(!options.length)return <div className="empty-inline">В справочнике пока нет активных специальностей.</div>;
  return <div className="global-supply-norms">
    <div className="object-supply-norm-toolbar">
      <label><span>Специальность</span><select value={specialtyId} onChange={event=>chooseSpecialty(event.target.value)}>{options.map(option=><option key={option.id} value={option.id}>{option.name}</option>)}</select></label>
    ...
    <div className="object-ppe-template-editor-panel">
      ...
      {canManage&&<div className="object-supply-norm-add"><select value={addItemId} onChange={event=>setAddItemId(event.target.value)}><option value="">Добавить позицию…</option>{available.map(item=><option key={item.id} value={item.id}>{item.name+" · "+(categoryLabels[item.category]??item.category)}</option>)}</select><button className="button" type="button" disabled={!addItemId} onClick={addItem}><Plus size={14}/> Добавить</button><button className="button primary" type="button" disabled={busy} onClick={()=>void save()}>{busy?"Сохраняем…":"Сохранить базовую норму"}</button></div>}
...
function draftFromTemplate(template:ObjectPpeTemplateRow|undefined):DraftRow[]{
  return (template?.items??[]).map(item=>({itemId:item.itemId,quantity:String(Number(item.quantity)),sizeSource:item.sizeSource,replacementCycleDays:item.replacementCycleDays==null?"":String(item.replacementCycleDays)}));
}
