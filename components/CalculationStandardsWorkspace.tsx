...
const tabs: [Tab, string, string][] = [
  ["models", "Модели расчёта", "Оформление, база выплат и комиссии"],
  ...
  ["rules", "Финансовые правила", "Маржа, резерв, НДС и округление по моделям"],
];
...
export function CalculationStandardsWorkspace({ models: initialModels, expenses: initialExpenses, schedules: initialSchedules, canManage }: { models: CalculationModelOption[]; expenses: ExpenseStandard[]; schedules: ScheduleStandard[]; canManage: boolean }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (!draft) return;
      setModels(draft.models); setExpenses(draft.expenses); setSchedules(draft.schedules);
  ...
  const persistDemo = (nextModels = models, nextExpenses = expenses, nextSchedules = schedules) => saveCompanyRulesDraft({ models: nextModels, expenses: nextExpenses, schedules: nextSchedules });
  ...
  const addAction = <button className="button primary" type="button" disabled={!canManage || (tab === "rules" && !active)} onClick={() => setEditor(tab === "rules" && active ? {kind:"model",action:"revise",item:active} : {kind:tab === "models" ? "model" : tab === "expenses" ? "expense" : "schedule",action:"create"})}>+ {tab === "models" ? "Модель" : tab === "rules" ? "Версия правил" : tab === "expenses" ? "Расход" : "График"}</button>;

  return <div className="standards-workspace">
    <div className="summary-strip"><span>Моделей <strong>{models.length}</strong></span><span>Расходов <strong>{expenses.length}</strong></span><span>Графиков <strong>{schedules.length}</strong></span><span>{demo ? <>Демо-контур <strong>изменения сохраняются в браузере</strong></> : <>Версии <strong>сохраняются в истории</strong></>}</span></div>
    ...
    {tab === "rules" && <section className="section"><div className="section-head"><div><h2>Финансовые правила модели</h2><p>Эти параметры входят в snapshot нового сценария. Для другой схемы создайте копию модели.</p></div>{active && <button className="button primary" disabled={!canManage} onClick={() => setEditor({kind:"model",action:"revise",item:active})}>Изменить правила</button>}</div><div className="standards-toolbar"><label>Модель <select value={active?.id ?? ""} onChange={event => setSelected(event.target.value)}>{models.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}</select></label></div>{editor?.kind === "model" && <ModelForm item={editor.item as CalculationModelOption | undefined} action={editor.action} onSubmit={save} onCancel={() => setEditor(null)}/>} {active && <RulesTable rules={active.rules}/>}</section>}
...
function rulesFromForm(form: FormData): CalculationRuleConfig { return { mandatoryChargePct:n(form.get("mandatoryChargePct")), mandatoryChargeFixedHourly:n(form.get("mandatoryChargeFixedHourly")), riskReservePct:n(form.get("riskReservePct")), minimumMarginPct:n(form.get("minimumMarginPct")), recommendedMarginPct:n(form.get("recommendedMarginPct")), vatPct:n(form.get("vatPct")), roundingStep:n(form.get("roundingStep"),1), legalParametersVerified:form.get("legalParametersVerified") === "on", payStructure:s(form.get("payStructure")) === "mrot_plus_supplement" ? "mrot_plus_supplement" : "full_pay", officialBasePerWorkerMonthly:n(form.get("officialBasePerWorkerMonthly")), mandatoryChargeBase:s(form.get("mandatoryChargeBase")) === "official_base" ? "official_base" : "full_pay", supplementCommissionPct:n(form.get("supplementCommissionPct")), supplementCommissionFixedPerWorkerMonthly:n(form.get("supplementCommissionFixedPerWorkerMonthly")) }; }
