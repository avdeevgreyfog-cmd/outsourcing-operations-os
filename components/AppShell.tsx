...
export async function AppShell({ actor, children }: { actor: Actor; children: React.ReactNode }) {
  ...
  return <div className="app-shell" data-initial-theme={theme} data-initial-ui={visualMode}>
    <script dangerouslySetInnerHTML={{ __html: `document.documentElement.dataset.theme=${JSON.stringify(theme)};document.documentElement.dataset.operisUi=${JSON.stringify(visualMode)}` }} />
