...
function PreparationFields({
  ...
  travelState:string;setTravelState:(value:any)=>void;ticketAssignee:string;setTicketAssignee:(value:string)=>void;ticketDue:string;setTicketDue:(value:string)=>void;
  housingState:string;setHousingState:(value:any)=>void;housingAssignee:string;setHousingAssignee:(value:string)=>void;housingDue:string;setHousingDue:(value:string)=>void;
  ...
  return <>
    ...
    <label>Размещение<select value={housingState} onChange={e=>setHousingState(e.target.value)}><option value="not_required">Не требуется</option><option value="needs_booking">{housingProvided?"Нужно забронировать место":"Нужно организовать жильё"}</option><option value="booked">Место подтверждено</option></select></label>
