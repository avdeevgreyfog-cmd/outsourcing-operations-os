  ...
  return <>
    ...
    <div className="workspace-grid" style={{marginTop:16}}>
      <Section title="Имущество к возврату" note="Возвратное имущество должно быть возвращено или списано до фактического завершения работы.">
        {context.outstandingAssets.length?<div className="request-table-wrap"><table className="data-table"><thead><tr><th>Позиция</th><th>Вариант</th><th>Количество</th></tr></thead><tbody>{context.outstandingAssets.map(row=><tr key={row.itemId+":"+row.variant}><td className="cell-title">{row.item}</td><td>{row.variant||"—"}</td><td className="num">{row.quantity} {row.unit}</td></tr>)}</tbody></table><div style={{padding:12}}><Link className="button" href={"/assets?worker="+workerId}>Открыть возврат / списание</Link></div></div>:<div className="empty-inline">Возвратного имущества на сотруднике нет</div>}
      </Section>
