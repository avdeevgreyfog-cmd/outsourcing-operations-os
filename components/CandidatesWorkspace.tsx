...
export function CandidatesWorkspace({rows,demo,exitReasons,canEdit,canConvert}:{rows:RecruitingApplicationRow[];demo:boolean;exitReasons:RecruitingOptions['exitReasons'];canEdit:boolean;canConvert:boolean}){
 ...
 <div className="recruiting-summary"><div><span>Людей в базе</span><strong>{people.size}</strong></div><div><span>Активно в подборе</span><strong>{new Set(all.filter(x=>isActiveStage(x.stage)).map(x=>x.candidateId)).size}</strong></div><div><span>Вышли</span><strong>{new Set(all.filter(x=>['first_shift','retention_7','retention_30'].includes(x.stage)).map(x=>x.candidateId)).size}</strong></div><div><span>Повторные обращения</span><strong>{[...people].filter(id=>all.filter(x=>x.candidateId===id).length>1).length}</strong></div></div>
