...
const objectStatusLabels:Record<string,string>={prelaunch:"Подготовка",launch:"Готов к запуску",active:"Активен",paused:"Приостановлен",completed:"Завершён",archived:"Архив"};
...
export function NeedsGrid({rows}:{rows:NeedRow[]}) { const columns:GridColumn<NeedRow>[]=[
  {key:"object",label:"Объект",sticky:true,width:220,render:x=><Link className="cell-title" href={`/objects/${x.objectId}?tab=needs`}>{x.object}</Link>},{key:"specialty",label:"Специальность"},{key:"required",label:"Нужно",numeric:true},{key:"filled",label:"Закрыто",numeric:true},{key:"deficit",label:"Дефицит",numeric:true},{key:"deadline",label:"Дедлайн"},{key:"status",label:"Статус",render:x=><Status tone={x.deficit?"warn":"good"}>{needStatusLabels[x.status]??"В работе"}</Status>}
