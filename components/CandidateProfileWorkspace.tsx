...
export function CandidateProfileWorkspace({profile,candidateId,demo,canEdit,canConvert,exitReasons}:Props){
 ...
 useEffect(()=>{
  ...
  try{
   ...
   const hydrated:CandidateProfile={
    ...
    status:profile?.status??(applications.some(x=>["first_shift","retention_7","retention_30"].includes(x.stage))?"worker":"active"),applications,
 ...
 return <div className="recruiting-workspace">
  ...
  </div><div><Section title="Текущий статус"><div style={{padding:"6px 15px 14px"}}>{latest?<><KeyValue label="Потребность" value={latest.need}/><KeyValue label="Объект" value={latest.object??"—"}/><KeyValue label="Этап" value={<Status tone={["first_shift","retention_7","retention_30"].includes(latest.stage)?"good":"info"}>{latest.stageLabel}</Status>}/><KeyValue label="Рекрутер" value={latest.owner??"—"}/><KeyValue label="Менеджер / мастер" value={latest.manager??"—"}/><KeyValue label="Следующее действие" value={`${latest.workflow?.nextActionText??""} ${formatWorkDate(latest.nextActionAt)}`}/></>:<span>Нет активных заявок</span>}</div></Section><Section title="Комментарий"><div style={{padding:"6px 15px 14px",fontSize:12,lineHeight:1.5}}>{current.notes??"Комментарий не добавлен"}</div></Section></div></div>}
  {tab==="applications"&&<Section title="Заявки кандидата" note="Один человек может участвовать в нескольких потребностях, история не теряется"><div className="candidate-applications">{current.applications.map(application=><div className="candidate-application-row" key={application.applicationId}><div><strong>{application.need}</strong><span>{application.object??"Без объекта"}</span></div><div><Status tone={["first_shift","retention_7","retention_30"].includes(application.stage)?"good":["rejected","no_show"].includes(application.stage)?"bad":"info"}>{application.stageLabel}</Status><span>{application.rejectionReason??""}</span></div><div><strong>{application.owner??"Без рекрутера"}</strong><span>{formatWorkDate(application.nextActionAt)}</span></div><div><button className="button" onClick={()=>setSelected(application)}>Этап и действия</button></div></div>)}</div></Section>}
