...
export function CandidateProfileWorkspace({profile,candidateId,options,needs,demo,canEdit,canConvert,exitReasons}:Props){
 ...
 return <div className="recruiting-workspace candidate-profile-v2">
  ...
  {tab==="overview"&&<div className="candidate-profile-grid candidate-profile-overview"><div>
   ...
   <Section title="Источник и атрибуция"><div className="candidate-current-state"><KeyValue label="Источник" value={current.source??"—"}/><KeyValue label="Канал" value={current.sourceChannel??"—"}/><KeyValue label="Кампания / объявление" value={current.sourceCampaign??"—"}/><KeyValue label="Кто привёл кандидата" value={current.originalRecruiter??"—"}/><KeyValue label="Ответственный сейчас" value={latest?.owner??current.currentRecruiter??"—"}/></div></Section>
   {current.workerId&&<Section title="Связь с сотрудником"><div className="candidate-worker-link"><UserCheck size={18}/><div><strong>Создана карточка сотрудника</strong><span>Дальнейшая работа ведётся в карточке сотрудника.</span></div>{!demo&&<Link className="button primary" href={`/workers/${current.workerId}`}>Открыть</Link>}</div></Section>}
  </div></div>}
...
function DocumentDossierEditorRow({row,canEdit,busy,onSave}:{row:CandidateProfile["documents"][number];canEdit:boolean;busy:boolean;onSave:(patch:{status?:string;note?:string|null})=>void}){
 ...
 return <div className="candidate-document-edit-row">
  <div className="candidate-document-copy"><strong>{row.name}</strong>{row.groupType==="clearance"&&[row.object,row.need].filter(Boolean).length>0&&<span>{[row.object,row.need].filter(Boolean).join(" · ")}</span>}{row.blocksProgress&&<span className="candidate-document-blocking">Блокирует переход</span>}{row.note&&!commentOpen&&<small className="candidate-document-note">{row.note}</small>}</div>
