...
export function CandidateProfileWorkspace({profile,candidateId,options,needs,demo,canEdit,canConvert,exitReasons}:Props){
 ...
 return <div className="recruiting-workspace candidate-profile-v2">
  ...
  {tab==="communications"&&<div className="candidate-profile-grid"><Section title="Хронология коммуникаций" note="Рабочая история контактов. Запись может относиться к человеку в целом или к конкретной заявке."><div className="candidate-history">{current.communications.length?current.communications.map(row=><div className="candidate-history-item" key={row.id}><time>{row.happenedAt}</time><div><strong>{row.author} · {communicationLabel(row.channel)}</strong><p>{row.summary}</p><div className="candidate-communication-context">{communicationContext(row.applicationId,current.applications)}</div></div></div>):<div className="empty-inline">Коммуникаций пока нет</div>}</div></Section><Section title="Добавить запись" note="Сначала фиксируется результат контакта, затем — конкретная договорённость или комментарий."><form className="recruiting-form" onSubmit={addCommunication}><label>Контекст<select value={comm.applicationId} onChange={e=>setComm(x=>({...x,applicationId:e.target.value}))}><option value="">Общая по кандидату</option>{current.applications.map(x=><option key={x.applicationId} value={x.applicationId}>{x.need} · {x.object??"без объекта"}</option>)}</select></label><label>Канал<select value={comm.channel} onChange={e=>setComm(x=>({...x,channel:e.target.value}))}><option value="phone">Телефон</option><option value="telegram">Telegram</option><option value="max">MAX</option><option value="whatsapp">WhatsApp</option><option value="email">Email</option><option value="meeting">Встреча</option><option value="note">Внутренняя заметка</option></select></label><label>Результат<select value={comm.result} onChange={e=>setComm(x=>({...x,result:e.target.value}))}><option value="contacted">Связались</option><option value="no_answer">Не дозвонились</option><option value="waiting">Ждём ответ</option><option value="documents">Документы / данные получены</option><option value="agreed">Договорились</option><option value="declined">Отказ / неактуально</option><option value="note">Внутренняя информация</option></select></label><label>Направление<select value={comm.direction} onChange={e=>setComm(x=>({...x,direction:e.target.value}))}><option value="outbound">Исходящая</option><option value="inbound">Входящая</option><option value="internal">Внутренняя</option></select></label><label className="wide">Комментарий / договорённость<textarea required value={comm.summary} onChange={e=>setComm(x=>({...x,summary:e.target.value}))} placeholder="Что произошло, о чём договорились, что важно знать следующему сотруднику"/></label><p className="candidate-communication-form-hint">Следующее действие и смену этапа фиксируйте через рабочий этап кандидата — так история и workflow не расходятся.</p><button className="button primary" disabled={busy==="communication"||(!canEdit&&!demo)}>Добавить запись</button></form></Section></div>}
...
function CandidateDocuments({profile,options,candidateId,demo,canEdit,onOpenStage}:{profile:CandidateProfile;options:RecruitingOptions;candidateId:string;demo:boolean;canEdit:boolean;onOpenStage:(application:RecruitingApplicationRow,stage:RecruitingStage|undefined)=>void}){
 ...
 return <div className="candidate-documents-workspace">
  {error&&<div className="recruiting-error" style={{gridColumn:"1 / -1"}}>{error}</div>}
  <Section title="Документы для трудоустройства" note="Единый пакет человека. Статус сохраняется при переводе на другую вакансию."><div className="candidate-dossier-list">{employment.map(row=><DocumentDossierEditorRow key={row.documentTypeId} row={row} canEdit={canEdit||demo} busy={busy===row.documentTypeId} onSave={patch=>void saveDocument(row,patch)}/>)}</div>{!employment.length&&<div className="empty-inline">Базовые документы ещё не настроены</div>}{addable.length>0&&<div className="candidate-document-add"><select value={addId} onChange={e=>setAddId(e.target.value)} disabled={!canEdit&&!demo}><option value="">Добавить другой документ из справочника</option>{addable.map(row=><option key={row.id} value={row.id}>{row.name}</option>)}</select><button type="button" className="button" disabled={!addId||(!canEdit&&!demo)||Boolean(busy)} onClick={()=>void addDocument()}>Добавить</button></div>}</Section>
  <Section title="Дополнительные документы и допуски" note="Требования конкретной вакансии. Ответственный и срок по оформляемым компанией документам доступны в рабочем этапе."><div className="candidate-dossier-list">{additional.map((row,index)=><DocumentDossierEditorRow key={`${row.documentTypeId}-${row.needId}-${index}`} row={row} canEdit={canEdit||demo} busy={busy===row.documentTypeId} onSave={patch=>void saveDocument(row,patch)}/>)}</div>{!additional.length&&<div className="empty-inline">Дополнительных требований нет</div>}</Section>
  {latest&&<div className="candidate-documents-stage"><div><strong>Текущий этап: {latest.stageLabel}</strong><span>{documentReadiness(latest,items)}</span></div><div className="candidate-current-actions"><button type="button" className="button" onClick={()=>onOpenStage(latest,undefined)}>Открыть рабочий этап</button>{next&&stageAdvanceActions.has(latest.stage+":"+next)&&<button type="button" className="button primary" onClick={()=>onOpenStage(latest,next)}>Перейти дальше</button>}</div></div>}
 </div>;
}
...
function DocumentDossierEditorRow({row,canEdit,busy,onSave}:{row:CandidateProfile["documents"][number];canEdit:boolean;busy:boolean;onSave:(patch:{status?:string;note?:string|null})=>void}){
 ...
 return <div className="candidate-document-edit-row"><div className="candidate-document-copy"><strong>{row.name}</strong><span>{row.groupType==="employment"?"Базовый документ":[row.need,row.object].filter(Boolean).join(" · ")||"Требование объекта"} · {providerLabel(row.provider)} · {deadlineLabel(row.requiredByStage)}{row.blocksProgress?" · блокирует":""}</span></div><select aria-label={"Статус: "+row.name} value={row.status} disabled={!canEdit||busy} onChange={e=>onSave({status:e.target.value})}>{statuses.map(status=><option key={status} value={status}>{documentStatusLabel(status)}</option>)}</select><textarea aria-label={"Комментарий: "+row.name} defaultValue={row.note??""} disabled={!canEdit||busy} placeholder="Комментарий по документу, если нужен" onBlur={e=>{if(e.currentTarget.value.trim()!==(row.note??"").trim())onSave({note:e.currentTarget.value.trim()||null})}}/></div>;
}
...
function documentReadiness(application:RecruitingApplicationRow,items:CandidateProfile["documents"]){const relevant=items.filter(row=>row.groupType==="employment"||(row.needId&&row.needId===application.needId));if(!relevant.length)return"Требования к документам не заданы";const ready=relevant.filter(row=>readyDocumentStatuses.has(row.status)).length;const blockers=relevant.filter(row=>row.blocksProgress&&!readyDocumentStatuses.has(row.status));return `Готово ${ready} из ${relevant.length}${blockers.length?` · блокируют: ${blockers.map(row=>row.name).join(", ")}`:" · блокирующих документов нет"}`}
