...
export function WorkspaceNavigation({ actor, sections, workspace }: { actor: Actor; sections: NavigationSection[]; workspace: WorkspaceContext }) {
  ...
  return <>
    ...
    <aside className={`sidebar ${compact ? "sidebar-compact" : ""} ${mobileOpen ? "sales-mobile-open" : ""}`}>
      <div className="brand"><span className="brand-mark"><Image src={`${assetBase}/operis-symbol.svg`} alt="" width={24} height={24} priority/></span>{!compact && <div><strong>OPERIS</strong><small>Операционная система</small></div>}</div>
    ...
    <header className={`topbar ${compact ? "topbar-compact" : ""}`}><button className="mobile-menu" type="button" onClick={() => { setPrefs(current => ({ ...current, compact: false })); setMobileOpen(v => !v); }} aria-label="Меню" aria-expanded={mobileOpen}><Menu size={18}/></button><div className="topbar-context"><span className="live-dot"/><strong>{activeSectionLabel}</strong><span className="topbar-separator">·</span><span>{currentRole}</span>{actor.accessPreview&&<span className="access-preview-flag">Режим проверки</span>}</div><div className="topbar-actions">{!staticGithubDemo&&<WorkspaceContextControls context={workspace} demo={actor.demo}/>} {!staticGithubDemo&&actor.demo&&<DemoRoleSwitch currentUserId={actor.userId}/>}<VisualModeToggle/><ThemeToggle/>{hasCapability(actor.access,"task.read")&&<Link className="icon-button" href="/tasks" aria-label="Задачи"><Activity size={16}/></Link>}<AccountMenu actor={actor}/></div></header>
