...
export function WorkspaceNavigation({ actor, sections }: { actor: Actor; sections: NavigationSection[] }) {
  ...
  const [openSections, setOpenSections] = useState<string[]>(activeSection ? [activeSection] : []); const [compact, setCompact] = useState(false); const [palette, setPalette] = useState(false); const [query, setQuery] = useState("");
  useEffect(() => { const timer=window.setTimeout(()=>{const saved=window.localStorage.getItem("operis.navigation");if(!saved)return;try{const value=JSON.parse(saved) as {sections?:string[];compact?:boolean};if(value.sections)setOpenSections(value.sections);if(typeof value.compact==="boolean")setCompact(value.compact)}catch{}},0);return()=>window.clearTimeout(timer)}, []);
  useEffect(() => { window.localStorage.setItem("operis.navigation", JSON.stringify({ sections: openSections, compact })); }, [openSections, compact]);
  ...
  return <>
    <aside className={`sidebar ${compact ? "sidebar-compact" : ""}`}>
      ...
      <nav className="nav-groups" aria-label="Основная навигация">{sections.map((section) => { const Icon = icons[section.icon as keyof typeof icons] ?? Activity; const expanded = openSections.includes(section.id); return <div className="nav-section" key={section.id}>
        ...
        {!compact && expanded && <div className="nav-section-body">{section.groups.map((group) => <div className="nav-subgroup" key={group.id}><div className="nav-subgroup-label">{group.label}</div><div>{group.items.map((item) => <Link key={item.href} href={item.href} className={sameRoute(pathname, item.href) ? "active" : ""}>{item.label}</Link>)}</div></div>)}</div>}
      </div>})}</nav>
