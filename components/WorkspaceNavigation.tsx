...
export function WorkspaceNavigation({ actor, sections }: { actor: Actor; sections: NavigationSection[] }) {
  ...
  const [openSections, setOpenSections] = useState<string[]>([]); const [openGroups, setOpenGroups] = useState<string[]>(activeGroup ? [activeGroup] : []); const [compact, setCompact] = useState(false); const [palette, setPalette] = useState(false); const [mobileOpen, setMobileOpen] = useState(false); const [query, setQuery] = useState("");
  ...
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") { event.preventDefault(); setPalette(true); } if (event.key === "Escape") { setPalette(false); setMobileOpen(false); } }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, []);
  ...
  return <>
    ...
    <aside className={`sidebar ${compact ? "sidebar-compact" : ""} ${mobileOpen ? "sales-mobile-open" : ""}`}>
      ...
      <nav className="nav-groups" aria-label="Основная навигация">{sections.map((section) => { const Icon = icons[section.icon as keyof typeof icons] ?? Activity; const expanded = openSections.includes(section.id) || activeSection === section.id; return <div className="nav-section" key={section.id}>
        <button type="button" className={`nav-section-button ${activeSection === section.id ? "is-current" : ""}`} onClick={() => { if (window.matchMedia("(max-width: 760px)").matches) { setCompact(false); setMobileOpen(true); } else if (compact) setCompact(false); toggleSection(section.id); }} title={section.label} aria-expanded={expanded}><Icon size={17}/>{!compact && <><span>{section.label}</span><ChevronDown size={14} className={expanded ? "rotated" : ""}/></>}</button>
        {!compact && expanded && <div className="nav-section-body">{section.groups.map((group) => { const groupOpen = openGroups.includes(group.id) || activeGroup === group.id; return <div className="nav-subgroup" key={group.id}><button type="button" onClick={() => toggleGroup(group.id)} aria-expanded={groupOpen}><span>{group.label}</span><ChevronDown size={13} className={groupOpen ? "rotated" : ""}/></button>{groupOpen && <div>{group.items.map((item) => { const active = sameRoute(pathname, item.href, currentView); return <Link key={item.id ?? item.href} href={item.href} onClick={() => setMobileOpen(false)} className={active ? "active" : ""} aria-current={active ? "page" : undefined}><span>{item.label}</span>{item.status === "foundation" && <small className="nav-foundation-badge">База</small>}</Link> })}</div>}</div>})}</div>}
      </div>})}</nav>
