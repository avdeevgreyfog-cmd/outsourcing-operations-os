...
ALTER TABLE requests ALTER COLUMN request_number SET DEFAULT ('З-' || to_char(now(),'YYMM') || '-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,6)));
...
UPDATE requests SET request_number = DEFAULT WHERE request_number IS NULL;
...
ALTER TABLE calculations ALTER COLUMN calculation_number SET DEFAULT ('Р-' || to_char(now(),'YYMM') || '-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,6)));
...
UPDATE calculations SET calculation_number = DEFAULT WHERE calculation_number IS NULL;
...
CREATE TABLE calculation_approvals (
  ...
  request_id uuid REFERENCES requests(id) ON DELETE CASCADE, scenario_id uuid NOT NULL REFERENCES calculation_scenarios(id) ON DELETE CASCADE,
  status text NOT NULL DEFAULT 'pending' CHECK (status IN ('pending','approved','rejected','revision_requested')),
  requested_by_user_id uuid NOT NULL REFERENCES app_users(id), assigned_to_user_id uuid REFERENCES app_users(id), decided_by_user_id uuid REFERENCES app_users(id),
  comment text, requested_at timestamptz NOT NULL DEFAULT now(), decided_at timestamptz, created_at timestamptz NOT NULL DEFAULT now()
);
...
ALTER TABLE proposals ALTER COLUMN proposal_number SET DEFAULT ('КП-' || to_char(now(),'YYMM') || '-' || upper(substr(replace(gen_random_uuid()::text,'-',''),1,6)));
...
UPDATE proposals SET proposal_number = DEFAULT WHERE proposal_number IS NULL;
...
CREATE TABLE request_public_links (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  request_id uuid REFERENCES requests(id) ON DELETE CASCADE, mode text NOT NULL CHECK (mode IN ('create','complete')), token_hash text NOT NULL UNIQUE,
  expires_at timestamptz, revoked_at timestamptz, created_by_user_id uuid NOT NULL REFERENCES app_users(id), created_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE commercial_rule_sets (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), organization_id uuid NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  name text NOT NULL, version integer NOT NULL, effective_from date NOT NULL, effective_to date, rules_json jsonb NOT NULL,
  created_by_user_id uuid NOT NULL REFERENCES app_users(id), created_at timestamptz NOT NULL DEFAULT now(), UNIQUE (organization_id, name, version)
);
...
ALTER TABLE calculation_approvals ENABLE ROW LEVEL SECURITY; ALTER TABLE calculation_approvals FORCE ROW LEVEL SECURITY;
...
ALTER TABLE request_public_links ENABLE ROW LEVEL SECURITY; ALTER TABLE request_public_links FORCE ROW LEVEL SECURITY;
...
ALTER TABLE commercial_rule_sets ENABLE ROW LEVEL SECURITY; ALTER TABLE commercial_rule_sets FORCE ROW LEVEL SECURITY;
...
('sales.request.edit','sales','request','edit',false,'Изменение заявки'),('sales.request.archive','sales','request','archive',false,'Архивирование и восстановление заявки'),
('sales.request.delete_draft','sales','request','delete_draft',false,'Удаление пустого ошибочного черновика'),('sales.request.margin.read','sales','request','margin.read',true,'Просмотр внутренней маржи заявки'),
('sales.request.public_link.manage','sales','request','public_link.manage',false,'Управление публичными ссылками заявки'),('calculation.approval.read','calculation','approval','read',false,'Просмотр согласований расчёта'),
('calculation.approval.decide','calculation','approval','decide',true,'Решение по согласованию расчёта'),('sales.proposal.read','sales','proposal','read',false,'Просмотр коммерческих предложений'),
('sales.proposal.create','sales','proposal','create',false,'Создание версии коммерческого предложения'),('sales.proposal.send','sales','proposal','send',false,'Отправка коммерческого предложения'),
('sales.proposal.accept','sales','proposal','accept',true,'Фиксация принятого коммерческого предложения'),('calculation.rules.manage','calculation','rules','manage',true,'Управление нормативами и правилами')
ON CONFLICT (capability) DO NOTHING;
INSERT INTO permission_grants(organization_id,role_template_id,capability,effect,scope_type)
SELECT rt.organization_id,rt.id,p.capability,'allow','all_org' FROM role_templates rt CROSS JOIN (VALUES ('sales.request.edit'),('sales.request.archive'),('sales.request.margin.read'),('sales.request.public_link.manage'),('calculation.approval.read'),('sales.proposal.read'),('sales.proposal.create'),('sales.proposal.send')) p(capability)
WHERE rt.code IN ('admin','owner','commercial_manager','sales_manager') ON CONFLICT DO NOTHING;
INSERT INTO permission_grants(organization_id,role_template_id,capability,effect,scope_type)
SELECT rt.organization_id,rt.id,p.capability,'allow','all_org' FROM role_templates rt CROSS JOIN (VALUES ('calculation.approval.decide'),('sales.proposal.accept'),('calculation.rules.manage'),('sales.request.delete_draft')) p(capability)
WHERE rt.code IN ('admin','owner','commercial_director','director') ON CONFLICT DO NOTHING;
COMMIT;
