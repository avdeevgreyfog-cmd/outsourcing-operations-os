...
export type TimesheetRatePeriod={kind:"any"|"day"|"night";amount:number|string;unit:"hour"|"shift"|"month";effectiveFrom:string;effectiveTo:string|null};
export type TimesheetAssignmentPeriod={specialtyId:string|null;specialty:string|null;effectiveFrom:string;effectiveTo:string|null};
export type TimesheetAbsenceRange={type:"intershift"|"vacation"|"sick"|"personal"|"other";from:string;to:string|null;returnDate:string|null};
export type TimesheetWorkerRow = { workerId:string; rowId?:string; conditionSegment?:boolean; name:string; rowKind?:"worker"|"candidate"; candidateId?:string|null; applicationId?:string|null; specialty?:string|null; effectiveFrom?:string|null; effectiveTo?:string|null; specialtyHistory?:TimesheetAssignmentPeriod[]; rateHistory?:TimesheetRatePeriod[]; absenceRanges?:TimesheetAbsenceRange[]; days?:Record<string,TimesheetCellValue>; dayCells?:Record<string,TimesheetCellValue>; nightCells?:Record<string,TimesheetCellValue>; plannedShiftKinds?:Record<string,"day"|"night"|"mixed">; plannedHours?:number|null; total:number|string; dayHours?:number|string|null; night?:number|string|null; overtime?:number|string|null; rate?:number|string|null; dayRate?:number|string|null; nightRate?:number|string|null; accrual?:number|string|null; dayAccrued?:number; nightAccrued?:number; calculatedAccrual?:number; premium?:number|null; adjustment?:number|null; accrualTotal?:number|null; paidAmount?:number|null; payableAmount?:number|null };
...
export type PaymentRow = ScopedRow & { id:string; workerId:string; worker:string; objectId?:string|null; object?:string|null; kind:"advance"|"payment"; purpose?:"advance"|"daily_shift"|"other"|null; workDate?:string|null; date?:string|null; dateIso?:string|null; amount:number|string; status:string; reference?:string|null; recordSource?:"finance"|"object_manager"|"integration"|"legacy"|null; reconciliationStatus?:"unreconciled"|"confirmed"|"conflict"|null; paymentMethod?:"transfer"|"cash"|"other"|null };
export type IncidentRow = ScopedRow & { id:string; objectId:string; object:string; title:string; type:string; occurredAt:string; occurredAtIso?:string|null; severity:string; status:string; responsibleUserId?:string|null; responsible?:string|null; workerId?:string|null; worker?:string|null; description:string; resolution?:string|null; financialEffectAmount?:number|string|null; financialEffectStatus?:"none"|"proposed"|"approved"|"rejected"|"applied"; financialEffectKind?:"worker_adjustment"|"company_expense"|"client_claim"|null; financialEffectBasis?:string|null };
  ...
  return withTenant(actor.organizationId, actor.userId, async (sql) => {
    const rows = await sql<WorkerRow[]>`
      SELECT w.id,w.origin_candidate_id "originCandidateId",w.organization_id "organizationId",w.full_name "fullName",w.phone,w.status,w.source,
             ...
             exit_plan.effective_date::text "plannedExitDate",exit_plan.reason_code "plannedExitReasonCode",
             next_assignment.effective_from::text "plannedTransferDate",next_object.name "plannedTransferObject",next_specialty.name "plannedTransferSpecialty",
             COALESCE(upcoming_shifts.dates,ARRAY[]::text[]) "upcomingShiftDates",
      ...
      LEFT JOIN LATERAL (SELECT effective_date,reason_code FROM worker_exit_processes x WHERE x.worker_id=w.id AND x.status='planned' ORDER BY x.effective_date LIMIT 1) exit_plan ON true
      LEFT JOIN LATERAL (SELECT x.object_id,x.specialty_id,x.effective_from FROM worker_object_assignments x WHERE x.worker_id=w.id AND x.effective_from>current_date ORDER BY x.effective_from LIMIT 1) next_assignment ON true
      LEFT JOIN objects next_object ON next_object.id=next_assignment.object_id
      LEFT JOIN specialties next_specialty ON next_specialty.id=next_assignment.specialty_id
      LEFT JOIN LATERAL (
        SELECT ARRAY(SELECT sh.shift_date::text FROM shift_assignments sa JOIN shifts sh ON sh.id=sa.shift_id
          ...
          ORDER BY sh.shift_date,sh.starts_at LIMIT 90) dates
      ) upcoming_shifts ON true
