  ...
  return withTenant(actor.organizationId, actor.userId, async (sql) => {
    const rows = await sql<any[]>`
      SELECT r.id, r.organization_id "organizationId", r.title, r.status, r.location_text location,
             ...
             r.schedule_json schedule_json, r.housing_rule housing, r.vat_mode vat,
...
export async function listCalculations(actor: Actor) {
...
export async function listObjects(actor: Actor) {
...
export async function listNeeds(actor: Actor) {
...
export async function listWorkers(actor: Actor) {
  if (actor.demo) {
...
export async function getTimesheet(actor: Actor) {
  ...
  return withTenant(actor.organizationId, actor.userId, async (sql) => {
    const [meta] = await sql<any[]>`SELECT id "objectId",name object,organization_id "organizationId",client_company_id "clientId",region_id "regionId",owner_user_id "ownerUserId" FROM objects ORDER BY created_at DESC LIMIT 1`;
    ...
    `;
    const [clientSnap] = await sql<any[]>`SELECT (snapshot_json->>'hours')::numeric hours,status FROM timesheet_snapshots WHERE object_id=${meta.objectId}::uuid AND view_type='client' ORDER BY period_end DESC LIMIT 1`;
...
export async function listTasks(actor: Actor) {
  ...
  return withTenant(actor.organizationId,actor.userId,async(sql)=>{
    const rows=await sql<any[]>`SELECT id,organization_id "organizationId",title,status,priority,assignee_user_id "ownerUserId",ARRAY[assignee_user_id::text] "assigneeUserIds",to_char(due_at,'DD.MM HH24:MI') due,entity_type entity,created_by_user_id "createdByUserId" FROM tasks ORDER BY due_at NULLS LAST`;
...
export async function getCommandCenter(actor: Actor) {
  ...
  return {
    objects: objectRows,
    tasks: taskRows.filter((x:any)=>x.status!=="done"),
    needs: needRows,
...
export async function listAccessUsers(actor: Actor) {
  ...
  return withTenant(actor.organizationId,actor.userId,async(sql)=>sql<any[]>`
...
export async function listAudit(actor: Actor, limit=20) {
