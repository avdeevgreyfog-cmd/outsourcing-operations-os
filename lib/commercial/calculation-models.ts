...
export async function getCalculationModels(actor: Actor): Promise<CalculationModelOption[]> {
  ...
  return withTenant(actor.organizationId, actor.userId, async (sql) => sql<CalculationModelOption[]>`
    SELECT m.id,m.name,m.code,
      rv.id "ruleVersionId",rv.version "ruleVersion",rv.source "ruleSource",COALESCE(rv.rules_json,'{}'::jsonb) rules
    FROM calculation_models m
    LEFT JOIN LATERAL (
      SELECT id,version,source,rules_json
