...
export type CommercialCalculationRow = {
  id:string;organizationId:string;sourceType:"request"|"tender";sourceId:string;source:string;
  ...
  billingUnit:string;pricingMode:string;marginPct:number|string;monthlyContribution:number|string;warnings:string[];ruleVersion:number|null;ruleSource:string|null;
...
export async function listCommercialCalculations(actor:Actor):Promise<CommercialCalculationRow[]>{
  ...
  if(actor.demo){
    return demo.calculations.map((item,index)=>({
      ...item,sourceType:"request" as const,sourceId:item.requestId,source:item.request,requestId:item.requestId,tenderId:null,request:item.request,
      ...
      billingUnit:"hour",pricingMode:"target_margin",warnings:[],ruleVersion:1,ruleSource:"Демонстрационная версия правил",clientId:null,
