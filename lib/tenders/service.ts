...
export type TenderOptions={
  ...
  platforms:string[];sources:string[];customers:string[];
  noBidReasons:Array<{code:string;name:string}>;lossReasons:Array<{code:string;name:string}>;
};
  ...
  return withTenant(actor.organizationId,actor.userId,async sql=>{
    const rows=await sql<TenderRow[]>`
      SELECT t.id,t.organization_id "organizationId",t.title,COALESCE(c.name,t.customer_name,'Заказчик не указан') customer,t.client_company_id "clientId",
        ...
        t.submission_deadline::text "submissionDeadline",t.initial_price "initialPrice",t.billing_unit "billingUnit",t.stage,t.decision,t.result,t.close_reason "closeReason",t.no_bid_reason_code "noBidReasonCode",t.no_bid_comment "noBidComment",t.result_reason_code "resultReasonCode",
...
export async function getTenderOptions(actor:Actor):Promise<TenderOptions>{
  if(actor.demo)return {clients:[],regions:[{id:MOW,name:"Москва и МО"}],specialties:[{id:"60000000-0000-4000-8000-000000000001",name:"Комплектовщик"},{id:"60000000-0000-4000-8000-000000000002",name:"Грузчик"}],members:[{userId:"10000000-0000-4000-8000-000000000002",membershipId:"50000000-0000-4000-8000-000000000002",name:"Илья Морозов",position:"Менеджер по продажам"},{userId:"10000000-0000-4000-8000-000000000006",membershipId:"50000000-0000-4000-8000-000000000006",name:"Елена Котова",position:"Экономист"}],legalEntities:[{id:"31000000-0000-4000-8000-000000000001",name:"ООО «Оперис Персонал»"}],companyDocuments:[{id:"b1000000-0000-4000-8000-000000000001",name:"Устав ООО «Оперис Персонал»",status:"active",expiresAt:null,legalEntityId:"31000000-0000-4000-8000-000000000001"}],platforms:["РТС-тендер","B2B-Center","Сбер А","ЕИС"],sources:["Список аналитика","Ручной ввод","Импорт"],customers:["ГБУ «Городские проекты»","АО «Логистик Групп»","ООО «Сервис Плюс»","АО «Промстрой»"],noBidReasons:[{code:"economics",name:"Низкая маржинальность / не проходит экономика"},{code:"deadline",name:"Не успеваем подготовить заявку"},{code:"staffing",name:"Нет нужного персонала / ресурсов"},{code:"payment",name:"Неподходящие условия оплаты"},{code:"requirements",name:"Завышенные требования к участнику"},{code:"security",name:"Обеспечение / гарантия слишком рискованны"},{code:"geography",name:"Не подходит география"},{code:"documents",name:"Нет необходимых документов / допуска"},{code:"tailored",name:"Признаки закупки под конкретного конкурента"},{code:"volume",name:"Недостаточный объём / потенциал"},{code:"other",name:"Другое"}],lossReasons:[{code:"price",name:"Цена"},{code:"score",name:"Проиграли по баллам / критериям"},{code:"qualification",name:"Опыт / квалификация"},{code:"documents",name:"Документы / комплектность заявки"},{code:"competitor",name:"Выбран другой участник"},{code:"timing",name:"Сроки / условия исполнения"},{code:"technical",name:"Техническое несоответствие"},{code:"rejected",name:"Заявка отклонена"},{code:"other",name:"Другое"}]};
  return withTenant(actor.organizationId,actor.userId,async sql=>{
    const [clients,regions,specialties,members,legalEntities,companyDocuments,platforms,sources,customers,noBidReasons,lossReasons]=await Promise.all([
      ...
      sql<Array<{value:string}>>`SELECT DISTINCT platform value FROM tenders WHERE platform IS NOT NULL AND btrim(platform)<>'' ORDER BY platform`,
      sql<Array<{value:string}>>`SELECT DISTINCT source_name value FROM tenders WHERE source_name IS NOT NULL AND btrim(source_name)<>'' ORDER BY source_name`,
      sql<Array<{value:string}>>`SELECT DISTINCT COALESCE(c.name,t.customer_name) value FROM tenders t LEFT JOIN client_companies c ON c.id=t.client_company_id WHERE COALESCE(c.name,t.customer_name) IS NOT NULL ORDER BY value`,
      sql<Array<{code:string;name:string}>>`SELECT code,name FROM tender_reason_catalog WHERE kind='no_bid' AND active ORDER BY sort_order,name`,
      sql<Array<{code:string;name:string}>>`SELECT code,name FROM tender_reason_catalog WHERE kind='lost' AND active ORDER BY sort_order,name`,
    ]);return {clients,regions,specialties,members,legalEntities,companyDocuments,platforms:platforms.map(item=>item.value),sources:sources.map(item=>item.value),customers:customers.map(item=>item.value),noBidReasons,lossReasons};
  });
}
