...
export function calculateCommercialScenario(input) {
  ...
  const workerPayMonthly = convertWorkerPayToMonthly(workerPayAmount, workerPayUnit, {
    workers, hoursPerWorker, shiftsPerWorker, unitsPerWorkerShift: positive(input.unitsPerWorkerShift, 1),
  });

  const payStructure = rules.payStructure === "mrot_plus_supplement" ? "mrot_plus_supplement" : "full_pay";
  const officialBaseMonthly = payStructure === "mrot_plus_supplement"
    ? Math.min(workerPayMonthly, nonNegative(rules.officialBasePerWorkerMonthly ?? 0) * workers)
    : workerPayMonthly;
  const supplementMonthly = Math.max(0, workerPayMonthly - officialBaseMonthly);
  const mandatoryChargeBaseMonthly = rules.mandatoryChargeBase === "official_base" ? officialBaseMonthly : workerPayMonthly;
  ...
  const configuredMandatoryMonthly = mandatoryChargeBaseMonthly * mandatoryChargePct / 100
    + mandatoryChargeFixedHourly * hoursPerWorker * workers;
  const supplementCommissionMonthly = supplementMonthly * nonNegative(rules.supplementCommissionPct ?? 0) / 100
    + nonNegative(rules.supplementCommissionFixedPerWorkerMonthly ?? 0) * workers;
  ...
  const additionalMonthly = (input.costs ?? []).filter((item) => item.enabled !== false)
    .reduce((sum, item) => sum + normalizeCostMonthly(item, {
      ...
      unitsPerWorkerShift: positive(input.unitsPerWorkerShift, 1), workerPayMonthly,
    }), 0);

  const reservePct = nonNegative(rules.riskReservePct ?? 0);
  const subtotalBeforeReserve = workerPayMonthly + configuredMandatoryMonthly + supplementCommissionMonthly + additionalMonthly;
  ...
  const pricingMode = input.pricingMode === "client_limit"
    ? "client_limit"
    : input.pricingMode === "target_profit"
      ? "target_profit"
      : "target_margin";
  const targetMonthlyContribution = nonNegative(input.targetMonthlyContribution ?? 0);
  const targetProfitPerBillingUnit = input.targetProfitPerBillingUnit == null ? null : nonNegative(input.targetProfitPerBillingUnit);
  ...
  const minimumVolumeMonthly = nonNegative(input.minimumVolumeMonthly ?? 0);
  ...
  const guaranteedVolumeMonthly = billingUnit === "project_fixed"
    ? billing.volumeMonthly
    : Math.max(
        billingUnit === "mixed" ? billing.variableVolumeMonthly : billing.volumeMonthly,
        minimumVolumeMonthly,
      );

  const profitTargetMonthly = targetProfitPerBillingUnit == null
    ? targetMonthlyContribution
    : targetProfitPerBillingUnit * Math.max(1, billingUnit === "mixed" ? billing.variableVolumeMonthly : billing.volumeMonthly);
  const targetRevenueMonthly = pricingMode === "target_profit"
    ? monthlyCost + profitTargetMonthly
    : monthlyCost / Math.max(0.05, 1 - targetMarginPct / 100);
  ...
  if (billingUnit === "mixed") {
    const variableVolume = guaranteedVolumeMonthly;
    ...
    revenueNetMonthly = billingUnit === "project_fixed"
      ...
      : clientRateNet * guaranteedVolumeMonthly;
  } else {
    clientRateNet = billingUnit === "project_fixed"
      ? targetRevenueMonthly * projectMonths
      : targetRevenueMonthly / Math.max(1, guaranteedVolumeMonthly);
    revenueNetMonthly = billingUnit === "project_fixed"
      ? clientRateNet / projectMonths
      : clientRateNet * guaranteedVolumeMonthly;
  }
  ...
  if (billingUnit !== "mixed" && billingUnit !== "project_fixed" && guaranteedVolumeMonthly > 0 && minimumMonthlyNet > 0) {
    clientRateNet = Math.max(clientRateNet, minimumMonthlyNet / guaranteedVolumeMonthly);
  }

  const roundingStep = positive(input.roundingStep ?? rules.roundingStep, 0.01);
  clientRateNet = roundForPricing(clientRateNet, roundingStep, pricingMode);
  if (variableRateNet !== null) variableRateNet = clientRateNet;

  if (billingUnit === "mixed") {
    revenueNetMonthly = Math.max(minimumMonthlyNet, fixedMonthlyNet + clientRateNet * guaranteedVolumeMonthly);
  } else if (billingUnit === "project_fixed") {
    revenueNetMonthly = Math.max(minimumMonthlyNet, clientRateNet / projectMonths);
  } else {
    revenueNetMonthly = Math.max(minimumMonthlyNet, clientRateNet * guaranteedVolumeMonthly);
  }
  ...
  const volumeForUnitCost = billingUnit === "mixed" ? guaranteedVolumeMonthly : billing.volumeMonthly;
  const costPerBillingUnit = billingUnit === "project_fixed"
    ? monthlyCost * projectMonths
    : monthlyCost / Math.max(1, volumeForUnitCost);
  const breakEvenRateNet = billingUnit === "project_fixed"
    ? monthlyCost * projectMonths
    : billingUnit === "mixed"
      ? Math.max(0, monthlyCost - fixedMonthlyNet) / Math.max(1, guaranteedVolumeMonthly)
      : monthlyCost / Math.max(1, guaranteedVolumeMonthly);
  ...
  return {
    pricingMode,
    billingUnit,
    billingUnitCode: input.billingUnitCode ?? null,
    billingUnitLabel: input.billingUnitLabel ?? null,
    ...
    guaranteedBillableVolumeMonthly: roundMoney(guaranteedVolumeMonthly),
    minimumVolumeMonthly: roundMoney(minimumVolumeMonthly),
    ...
    payStructure,
    officialBaseMonthly: roundMoney(officialBaseMonthly),
    supplementMonthly: roundMoney(supplementMonthly),
    mandatoryChargeBaseMonthly: roundMoney(mandatoryChargeBaseMonthly),
    mandatoryChargesMonthly: roundMoney(configuredMandatoryMonthly),
    supplementCommissionMonthly: roundMoney(supplementCommissionMonthly),
    ...
    targetMonthlyContribution: roundMoney(targetMonthlyContribution),
    targetProfitPerBillingUnit: targetProfitPerBillingUnit == null ? null : roundMoney(targetProfitPerBillingUnit),
...
function convertWorkerPayToMonthly(amount, unit, context) {
  ...
  if (unit === "unit") return amount * context.unitsPerWorkerShift * context.shiftsPerWorker * context.workers;
...
function normalizeCostMonthly(item, context) {
  ...
  switch (item.base) {
    ...
    case "per_worker_period": return amount * context.workers / Math.max(1, nonNegative(item.amortizationMonths ?? item.amortization_months ?? 1, 1));
    case "percent_of_worker_pay": return context.workerPayMonthly * amount / 100;
    case "role_month": return amount;
    case "role_fixed": return amount / Math.max(1, context.projectMonths);
...
function roundForPricing(value, step, pricingMode) {
  if (!Number.isFinite(value)) return 0;
  const safeStep = positive(step, 0.01);
  const scaled = value / safeStep;
  return (pricingMode === "client_limit" ? Math.floor(scaled + Number.EPSILON) : Math.ceil(scaled - Number.EPSILON)) * safeStep;
}
